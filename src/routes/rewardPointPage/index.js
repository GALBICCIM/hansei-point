    ...
    return (<div className={style.RewardPointPage}>
        ...
        <div className={style.RewardPointBody}>
            <div className={style.RewardPointInput}>
                <input value={phoneNum} readOnly />
                <p><a>이용약관</a>과 <a>개인정보 취급방침</a>에 동의하시면 휴대전화 번호 입력 후 아래 적립 버튼을 터치하세요.</p>
            </div>
            <div className={style.RewardPointPad}>
                {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((value) => <button key={`RewardPointPadButton-${value}`} className={style.RewardPointPadButton} value={value} onClick={numClick}>{value}</button>)}
                <button className={style.RewardPointPadButton} onClick={backClick}>←</button>
                <button className={style.RewardPointPadButton} value={0} onClick={numClick}>0</button>
                <button className={style.RewardPointPadButton} disabled={!(/^01([0|1|6|7|8|9])-?([0-9]{3,4})-?([0-9]{4})$/.test(phoneNum))} onClick={setPoint}>적립</button>
            </div>
        </div>
    </div>);
};
